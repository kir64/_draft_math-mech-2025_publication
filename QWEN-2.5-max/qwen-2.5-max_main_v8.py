...
def find_pet_by_tags(tags):
    """Находит питомцев по тегам через GET /pet/findByTags."""
    response = requests.get(f"{BASE_URL}/pet/findByTags", params={"tags": tags})
    return response
...
def update_pet_with_form(pet_id, name=None, status=None):
    """Обновляет питомца через POST /pet/{petId}."""
    data = {}
    if name:
        data["name"] = name
    if status:
    ...
    response = requests.post(f"{BASE_URL}/pet/{pet_id}", data=data, headers=headers)
    return response
...
def test_find_pets_by_invalid_status():
    """Тест поиска питомцев по невалидному статусу."""
...
def test_find_pets_by_tags():
    """Тест поиска питомцев по тегам."""
    response = find_pet_by_tags(["tag1"])
    assert response.status_code == 200
...
def test_upload_pet_image():
    ...
    assert response.status_code == 415  # API возвращает 415 Unsupported Media Type
...
def test_update_pet_with_form():
    ...
    assert response.status_code in [200, 400]  # API может вернуть 400 при неверных данных
...
def print_metrics_after_tests(request):
    def print_metrics():
    ...
    request.addfinalizer(print_metrics)
